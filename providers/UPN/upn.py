...
class UPN:
    ...
    def __init__(
        ...
        if len(year_allocation) != 2 or not year_allocation.isnumeric():
            raise ValueError("The Year Allocation must be the last two digits of the year (e.g. 20xx)")
        if len(serial_number) != 3:
            raise ValueError("The Serial Number must be three characters long, with at least the first two being numbers")
    ...
    def _check_letter(self):
        total = 0
        base_upn = self.la_code + self.establishment_number + self.year_allocation + self.serial_number
